...
static void BM_TaskSchedGlobal(benchmark::State& state, Task workload)
{
	...
	for (auto _ : state)
	{
		...
		ThreadPool<Task,GlobalWorkCoordinator<Task>> thread_pool(queue_size,num_workers);
...
BENCHMARK_CAPTURE( BM_TaskSchedGlobal,	// benchmarking function
